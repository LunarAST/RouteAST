...
*   **路径分割通用规则**：所有适配器在将原始路径字符串拆分为路由段数组时，**必须首先剔除首尾斜杠**（Slash），再按 `/` 分割，从物理上杜绝首位产生空字面量段（Empty Literal Segment）导致的索引越界、解析错误和对齐偏位。此规则适用于所有 HTTP 路径（REST、gRPC、Nginx 转发规则），不限于 gRPC。
...
- [ ] 所有路径提取时首要执行 Slash 去除过滤（剔除首尾斜杠），杜绝首位空字面量段。
